    ...
    return (
        <div className={`${isDeleteNotification ? "bg-black/52" : "bg-stone-100"} relative flex flex-col gap-4 px-5 w-full min-h-full max-h-full  overflow-hidden`}>
            <div className={`${!isDeleteNotification ? "opacity-0 scale-80" : "opacity-100 scale-100"} absolute top-1/2 left-1/2 z-30 -translate-x-1/2 -translate-y-1/2 transition-all duration-300`}>
                <Popup heading="Delete Category?" message={`Are you sure you want to delete this '${deleteCategory?.name}'`} setIsDeleteNotification={setIsDeleteNotification} deleteCategory={deleteCategory} handleDelete={handleDeleteCategory} deleteId={deleteCategory?._id} />
            </div>
                            ...
                            <div className='max-h-full overflow-auto'>
                                {
                                    category.map((item, index) => (
                                        <div>
                                            <div className={`p-5 w-full grid items-center gap-2 grid-cols-[100px_300px_1fr_1fr_1fr_1fr] ${index % 2 == 0 && "bg-gray-100"}`}>
                                                <div><input type='checkbox' className='scale-150' /></div>

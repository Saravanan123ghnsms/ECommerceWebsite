...
const LoginInfo = () => {
    ...
    const { isLogin, loginEmail, setLoginEmail, loginPassword, setLoginPassword, setIsFaEyeOpen, isFaEyeOpen, loginUser } = context;
            ...
            <div className={`loginSpace ${!isLogin ? 'loginPageHide' : 'loginPageShow'}`}>
                ...
                <form onSubmit={(e) => loginUser(e)}>
                    <div className='emailDiv'>
                        <input required type='text' name='email' style={{ display: 'block' }} value={loginEmail} onChange={(e) => { setLoginEmail(e.target.value) }} />
                        <label htmlFor='email' hidden={loginEmail.length > 0 ? true : false}>Email Address</label>
                    </div>

                    <div className='passwordDiv'>
                        <input required type={isFaEyeOpen ? 'text' : 'password'} name='password' style={{ display: 'block' }} value={loginPassword} onChange={(e) => {
